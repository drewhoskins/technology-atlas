...
def build_index(entries: list[dict], by_id: dict[str, dict]) -> str:
    ...
    for e in entries:
        by_type.setdefault(e.get("entry_type", "other"), []).append(e)
    for k in by_type:
        by_type[k].sort(key=lambda e: e["name"])

    sections = []
    for t in ENTRY_TYPE_ORDER:
        if t not in by_type:
            continue
        entries_in_type = by_type[t]
        cards = []
        for e in entries_in_type:
            desc = (e.get("description") or "").strip()
            # Up to ~2 sentences for the card preview.
            preview = ". ".join(desc.split(". ")[:2])
            if preview and not preview.endswith("."):
                preview += "."
            preview = preview[:320]
            year_hint = ""
            kd = e.get("key_dates", []) or []
            if kd:
                years = [d.get("year") for d in kd if d.get("year")]
                if years:
                    year_hint = f'<span class="mono year">{esc(min(years))}</span>'
            cards.append(f"""
      <li class="entry-card">
        <a href="./entries/{esc(slug(e["id"]))}.html">
          <div class="card-head">
            <span class="card-name">{esc(e["name"])}</span>
            {year_hint}
          </div>
          <div class="card-meta">
            <span class="badge domain">{esc(e.get("domain", ""))}</span>
            <span class="mono entry-id-small">{esc(e["id"])}</span>
          </div>
          <p class="card-preview">{esc(preview)}</p>
        </a>
      </li>""".rstrip())
        sections.append(f"""
<section class="type-section type-{esc(t)}">
  <h2>{esc(ENTRY_TYPE_LABELS.get(t, t.capitalize()))}</h2>
  <p class="type-blurb">{esc(ENTRY_TYPE_BLURBS.get(t, ""))}</p>
  <ul class="entry-grid">{"".join(cards)}
  </ul>
</section>""")
...
<main class="index">
  <header class="atlas-header">
    ...
    <h1>The Tech Atlas</h1>
    <p class="lede">
      Every entry below is a single innovation, traced through its predecessors,
      enabling components, funders, regulatory moments, geographic diffusion, and
      key dates. Every fact carries an inline citation back to a verbatim source
      excerpt — click through any entry, then click any superscript numeral to
      land on the supporting quotation.
    </p>
    <div class="divider" aria-hidden="true">&#8258;</div>
  </header>
  {"".join(sections)}
  <footer class="index-footer">
    <p class="footer-meta">
      <span class="mono">{len(entries)} entries</span>
      <span class="footer-sep">&middot;</span>
      <span>generated by <code>scripts/build_site.py</code></span>
    </p>
  </footer>
</main>
"""
    head = page_head("Index", "./styles.css")
...
def main() -> int:
    ...
    # 5. Index links to every entry.
    index_links = collect_internal_links(index_path)
    expected = {f"./entries/{slug(e['id'])}.html" for e in seeds}
    missing = expected - set(index_links)
    if missing:
        print(f"  index missing links to: {sorted(missing)}")
        fail += len(missing)
